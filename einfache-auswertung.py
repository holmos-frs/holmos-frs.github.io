...
fourier = (np.fft.fftshift(np.fft.fft2(image))).astype(np.complex128)
magnitude_spectrum = np.log(np.abs(fourier+1e-9))
print(magnitude_spectrum.dtype)

# ar = array.array(plt.Axes.get_xbound(plt.imshow(magnitude_spectrum)))
# ax.Axes.get_xbound(_AxesBas)
plt.imshow(magnitude_spectrum)
plt.show()
# print(plt.Axes.get_xbound)
# jetzt wirds hässlich
rect_center_x = int(input('Mittelpunkt X-Koordinate: '))

rect_center_y = int(input('Mittelpunkt Y-Koordinate: '))

rect_center_radius_x = int(input('Radius Rechteck X: '))
rect_center_radius_y = int(input('Radius Rechteck Y: '))

# FFT
cropped_fourier = np.zeros([height, width], dtype=complex)
satellite = fourier[rect_center_y - rect_center_radius_y:rect_center_y + rect_center_radius_y,
                    rect_center_x - rect_center_radius_x:rect_center_x + rect_center_radius_x]
cropped_fourier[height//2 - rect_center_radius_y:height//2 + rect_center_radius_y,
                width//2 - rect_center_radius_x:width//2 + rect_center_radius_x] = satellite
phaseangle = (np.angle(np.fft.ifft2(np.fft.fftshift(cropped_fourier)))).astype(np.float64)
...
plt.imsave(fname='unwrapped.png', arr=unwrapped_phase, format='png', cmap='gray')
...
satellite_patch = matplotlib.patches.Rectangle((rect_center_x - rect_center_radius_x, rect_center_y - rect_center_radius_y),
                                              rect_center_radius_x*2, rect_center_radius_y*2)
